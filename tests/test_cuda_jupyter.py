...
# Test matrix for cuda-jupyter images
CUDA_JUPYTER_MATRIX = [
    # CUDA 11.8
    ...
    # CUDA 12.6
    pytest.param("12.6.1", "22.04", "3.9", marks=[pytest.mark.cuda12]),
    pytest.param("12.6.1", "22.04", "3.10", marks=[pytest.mark.cuda12]),
    pytest.param("12.6.1", "22.04", "3.11", marks=[pytest.mark.cuda12]),
    pytest.param("12.6.1", "22.04", "3.12", marks=[pytest.mark.cuda12]),
...
@pytest.mark.cuda_jupyter
@pytest.mark.slow
def test_minimal_cuda_jupyter_build(project_root, docker_build_timeout, cleanup_docker_images, is_arm_mac):
    """Test a minimal cuda-jupyter build configuration."""
    if is_arm_mac:
        pytest.skip("Skipping x86_64 CUDA image build on ARM Mac.")
        
    tag = "cuda-jupyter-test:minimal"
    cleanup_docker_images(tag)
